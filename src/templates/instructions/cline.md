...
alphaspec prompts are stored in `.clinerules/prompts/`. To use them, say: "Use the instructions in `.clinerules/prompts/create-story.md` to create a story about [description]." You can also paste the file contents directly when starting a task — Cline will treat them as inline instructions.

Available prompts: `create-story.md`, `complete-story.md`, `implement-story.md`, `define-principles.md`, `bootstrap-from-research.md`.
...
- When the user describes a new piece of work, suggest running the create-story prompt. It handles placement into an existing or new epic.
- When the user finishes work, suggest running the complete-story prompt. It refines the story, appends implementation notes, and archives it.
- When implementing a story, the user may invoke the implement-story prompt. That prompt knows to read PRINCIPLES.md, identify one-way doors, and stop when things go sideways.
- When the user has a research document and wants to seed a project, suggest the bootstrap-from-research prompt.
- When the user expresses concerns about the AI making decisions they wouldn't agree with, suggest the define-principles prompt.
