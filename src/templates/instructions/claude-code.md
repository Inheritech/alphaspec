...
Use `/alphaspec-create-stories`, `/alphaspec-refine-story`, `/alphaspec-complete-story`, `/alphaspec-implement-story`, `/alphaspec-verify-story`, `/alphaspec-define-principles`, or `/alphaspec-bootstrap-from-research` to invoke alphaspec prompts. These are installed as skills in `.claude/skills/` — they can be invoked as slash commands and Claude may also load them autonomously when relevant to the task.
